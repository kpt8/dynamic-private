...
UniValue testdecrypt(const JSONRPCRequest& request) 
{
     // a. Use a hardcoded known Ed25519 private key seed
    const std::vector<unsigned char> vchPrivKeySeed{
    ...
    // b. Use a hardcoded known ciphertext to cause a segfault.
    const std::vector<unsigned char> vchEncryptedData{
    ...
    bool decryptStatus = DecryptBDAPData(vchPrivKeySeed, vchEncryptedData, vchDecrypted, strErrorMessage);

    if (decryptStatus) {
        LogPrintf("%s -- %s\n", __func__, stringFromVch(vchDecrypted));
    }
    else {
        LogPrintf("%s -- DecryptBDAPData failed, Error = %s\n", __func__, strErrorMessage);
        return "failed, Error = " + strErrorMessage;
    }
    
    std::vector<unsigned char> vchData, vchHash;
    CScript scriptData;
    scriptData << OP_RETURN << vchDecrypted;
    CLinkRequest link;
    if (GetBDAPData(scriptData, vchData, vchHash)) {
        link.UnserializeFromData(vchData, vchHash);
        return link.RequestorFQDN();
    }
    return "failed";
}
...
{ //  category              name                     actor (function)               okSafe argNames
  //  --------------------- ------------------------ -----------------------        ------ --------------------
    ...
    { "bdap",            "testdecrypt",              &testdecrypt,                  true, {""} },
#endif // ENABLE_WALLET
