...
public:
    ...
    std::string operator()(const CExtKeyPair &ek) const { return CDynamicAddress(ek, m_bech32).ToString(); }
    std::string operator()(const CStealthAddress &sxAddr) const { return CDynamicAddress(sxAddr, m_bech32).ToString(); }
};
...
public:
    CDynamicAddressVisitor(CDynamicAddress* addrIn, bool fBech32_ = false) : addr(addrIn), fBech32(fBech32_) {}
...
bool CDynamicAddress::Set(const CKeyID& id, bool fBech32)
{
...
bool CDynamicAddress::Set(const CScriptID& id, bool fBech32)
{
...
bool CDynamicAddress::Set(const CKeyID &id, CChainParams::Base58Type prefix, bool fBech32)
{
    SetData(fBech32 ? Params().Bech32Prefix(prefix) : Params().Base58Prefix(prefix), &id, 20);
    return true;
}

bool CDynamicAddress::Set(const CStealthAddress &sx, bool fBech32)
{
...
bool CDynamicAddress::Set(const CExtKeyPair &ek, bool fBech32)
{
...
bool CDynamicAddress::Set(const CTxDestination& dest, bool fBech32)
{
    return boost::apply_visitor(CDynamicAddressVisitor(this, fBech32), dest);
}
