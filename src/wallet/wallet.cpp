...
CWallet* CWallet::CreateWalletFromFile(const std::string walletFile)
{
    ...
    }
    {
        ...
        walletInstance->ExtKeyLoadMaster();
        walletInstance->ExtKeyLoadAccounts();
        walletInstance->ExtKeyLoadAccountPacks();
        walletInstance->LoadStealthAddresses();
        walletInstance->PrepareLookahead(); // Must happen after ExtKeyLoadAccountPacks
...
{
    if (IsLocked()) {
        LogPrintf("%s -- Wallet must be unlocked to derive hardened keys.", __func__);
        return nullptr;
    }
    CWalletDB* pwdb;
    if (IsCrypted()) {
...
int CWallet::ExtKeyNew32(CExtKey& out)
{
    if (LogAcceptCategory("hdwallet")) {
        LogPrintf("CWallet::ExtKeyNew32 from random.\n");
    }
...
{
    return ExtKeyUnlock(sea, MasterKey());
};

int CWallet::ExtKeyUnlock(CStoredExtKey* sek)
{
    return ExtKeyUnlock(sek, MasterKey());
};
...
int CWallet::ExtKeyEncrypt(CExtKeyAccount* sea, const CKeyingMaterial& vMKey, bool fLockKey)
{
    ...
    for (it = sea->vExtKeys.begin(); it != sea->vExtKeys.end(); ++it) {
        ...
        if (!sek->kp.IsValidV() && LogAcceptCategory("hdwallet")) {
...
int CWallet::ExtKeyImportLoose(CStoredExtKey& sekIn, CKeyID& idDerived, bool fBip44, bool fSaveBip44)
{
    ...
    if (pwdb->ReadExtKey(id, sekExist)) {
        ...
        sek = sekExist;
        if (!sek.kp.IsValidV() && sekIn.kp.IsValidV()) {
            sek.kp = sekIn.kp;
...
int CWallet::ExtKeySetMaster(CKeyID& idNewMaster)
{
    if (LogAcceptCategory("hdwallet")) {
    ...
    if (mvi != pEKNewMaster->mapValue.end() && mvi->second.size() == 1 && mvi->second[0] == EKT_BIP44_MASTER) {
        if (fNew) {
            delete pEKNewMaster;
        }
...
int CWallet::ExtKeyCreateAccount(CStoredExtKey* sekAccount, CKeyID& idMaster, CExtKeyAccount& ekaOut, const std::string& sLabel)
{
    ...
    if (IsCrypted() && ExtKeyEncrypt(&ekaOut, MasterKey(), false) != 0) {
        delete sekExternal;
        delete sekInternal;
        if (sekStealth) {
            delete sekStealth;
...
int CWallet::ExtKeySaveAccountToDB(const CKeyID& idAccount, CExtKeyAccount* sea)
{
...
int CWallet::ExtKeySetDefaultAccount(CKeyID& idNewDefault)
{
...
int CWallet::ExtKeyNewIndex(const CKeyID& idKey, uint32_t& index)
{
    if (LogAcceptCategory("hdwallet")) {
        CDynamicAddress addr;
...
int CWallet::ExtKeyGetIndex(CExtKeyAccount* sea, uint32_t& index, bool& fUpdate)
{
...
int CWallet::NewStealthAddress(CEKAStealthKey& akStealthOut, uint32_t nPrefixBits, const char* pPrefix, bool fBech32)
{
    ...
    if (!AddKeyPubKey(kSpend, kSpend.GetPubKey())) {
        return errorN(1, "%s -- Failed to save spend key.", __func__);
    }
    ...
    uint32_t idIndex;
    bool requireUpdateDB;
    if (0 == ExtKeyGetIndex(sea, idIndex, requireUpdateDB)) {
    ...
    if (!pwdb->WriteStealthAddress(sxAddr)) {
        ...
        LogPrintf("%s - Failed to save stealth address %s\n", __func__, sxAddr.ToString());
    }

    SetAddressBook(sxAddr, sLabel, "receive");
    LogPrintf("%s - Created stealth address %s\n", __func__, sxAddr.ToString());
    akStealthOut = aks;
    return 0;
};
...
{
    // Must scall WriteExtAccount after
...
bool CWallet::IndexStealthKey(CWalletDB* pwdb, uint160& hash, const CStealthAddressIndexed& sxi, uint32_t& id)
{
    ...
    if (!pwdb->ReadFlag("sxLastI", (int32_t&)id)) {
        LogPrintf("Warning: %s - ReadFlag sxLastI failed.\n", __func__);
    }
    ...
    if (!pwdb->WriteStealthAddressIndex(id, sxi) || !pwdb->WriteStealthAddressIndexReverse(hash, id) || !pwdb->WriteFlag("sxLastI", (int32_t&)id)) {
...
bool CWallet::UpdateStealthAddressIndex(const CKeyID& idK, const CStealthAddressIndexed& sxi, uint32_t& id)
{
    LOCK(cs_wallet);
    ...
    if (pwdb->ReadStealthAddressIndexReverse(hash, id)) {
        if (!pwdb->WriteStealthAddressLink(idK, id)) {
            return error("%s: WriteStealthAddressLink failed.\n", __func__);
        }
        return true;
    }

    if (!IndexStealthKey(pwdb, hash, sxi, id)) {
...
bool CWallet::GetStealthAddressScanKey(CStealthAddress& sxAddr) const
{
...
bool CWallet::GetStealthLinked(const CKeyID& idK, CStealthAddress& sx)
{
    LOCK(cs_wallet);
...
bool CWallet::ProcessStealthOutput(const CTxDestination& address, std::vector<uint8_t>& vchEphemPK, 
                                    uint32_t prefix, bool fHavePrefix, CKey& sShared, bool fNeedShared)
{
    LogPrintf("%s -- addresss = %s\n", __func__, CDynamicAddress(address).ToString());
    LOCK(cs_wallet);
    ...
    for (it = stealthAddresses.begin(); it != stealthAddresses.end(); ++it) {
        ...
        LogPrintf("%s -- Found stealth txn to address %s\n", __func__, it->Encoded());
        ...
        if (IsLocked()) {
            ...
            LogPrint("hdwallet", "%s: Wallet locked, adding key without secret.\n", __func__);
            // Add key without secret
            std::vector<uint8_t> vchEmpty;
            AddCryptedKey(pkE, vchEmpty);
            CDynamicAddress coinAddress(idExtracted);

            CPubKey cpkEphem(vchEphemPK);
            CPubKey cpkScan(it->scan_pubkey);
            CStealthKeyMetadata lockedSkMeta(cpkEphem, cpkScan);

            if (!pwdb->WriteStealthKeyMeta(idExtracted, lockedSkMeta)) {
                LogPrintf("%s: WriteStealthKeyMeta failed for %s.\n", __func__, coinAddress.ToString());
            }
        ...
        if (!GetKey(it->GetSpendKeyID(), sSpend)) {
            LogPrintf("%s: GetKey() for stealth spend key failed.\n", __func__,  CDynamicAddress(it->GetSpendKeyID()).ToString());
            return false;
        }
        ...
        for (AccStealthKeyMap::iterator it = ea->mapStealthKeys.begin(); it != ea->mapStealthKeys.end(); ++it) {
            ...
            if (LogAcceptCategory("hdwallet")) {
                ...
                if (!IsLocked() && !(ea->nFlags & EAF_HARDWARE_DEVICE)) {
                    ...
                    CDynamicAddress coinAddress(kTestId);
            ...
            if (0 != ExtKeySaveKey(pwdb, ea, ckidMatch, kNew)) {
    ...
    LogPrintf("%s -- Failed to find key for stealth address %s\n", __func__, CDynamicAddress(address).ToString());
    return false;
};
...
bool CWallet::ScanForOwnedOutputs(const CTransaction& tx, mapValue_t& mapNarr)
{
    LogPrintf("%s -- tx = %s\n", __func__, tx.ToString());
    AssertLockHeld(cs_wallet);
...
{
    LogPrintf("%s -- Loading master ext key.\n", __func__);
    LOCK(cs_wallet);
    ...
    if (!pwdb->ReadNamedExtKeyId("master", idMaster)) {
        int nValue;
        if (!pwdb->ReadFlag("madeDefaultEKey", nValue) || nValue == 0) {
            LogPrintf("%s -- Warning: ReadFlag madeDefaultEKey failed.\n", __func__);
        }
        LogPrintf("%s -- Warning: No master ext key has been set.\n", __func__);
        return 1;
    }

    pEKMaster = new CStoredExtKey();
    if (!pwdb->ReadExtKey(idMaster, *pEKMaster)) {
        delete pEKMaster;
        pEKMaster = nullptr;
        return errorN(1, "%s ReadExtKey failed to read master ext key.", __func__);
    }

    if (!pEKMaster->kp.IsValidP()) { // wallet could be locked, check pk
        delete pEKMaster;
        pEKMaster = nullptr;
        return errorN(1, "%s Loaded master ext key is invalid %s.", __func__, pEKMaster->GetIDString58());
    }
...
int CWallet::ExtKeyLoadAccountKeys(CWalletDB* pwdb, CExtKeyAccount* sea)
{
    ...
    for (size_t i = 0; i < sea->vExtKeyIDs.size(); ++i) {
        ...
        if (pwdb->ReadExtKey(id, *sek)) {
            ...
            LogPrintf("%s -- WARNING: Could not read key %d of account %s\n",__func__, i, sea->GetIDString58());
...
{
    LogPrintf("%s -- Loading ext accounts.\n", __func__);
    ...
    if (!pwdb->ReadNamedExtKeyId("defaultAccount", idDefaultAccount)) {
        LogPrintf("%s -- Warning: No default ext account set.\n", __func__);
    }
    // Load Ext Key Accounts
    std::vector<std::pair<CKeyID, CExtKeyAccount*>> vPairKeyAccounts;
    if (!pwdb->LoadExtKeyAccounts(vPairKeyAccounts, nTimeFirstKey)) {
        LogPrintf("%s -- Error getting ext key accounts from wallet db.\n", __func__);
    }

    for (const std::pair<CKeyID, CExtKeyAccount*>& keyAccount : vPairKeyAccounts) {
        CKeyID idAccount = keyAccount.first;
        CExtKeyAccount* sea = keyAccount.second;
...
{
    LogPrintf("%s Loading ext account packs.\n", __func__);
    ...
    if (!pwdb)
        return errorN(1, "%s: GetWalletDB failed.", __func__);
    // Load Ext Key Packs (CEKAKeyPack)
    std::vector<std::pair<CKeyID, std::vector<CEKAKeyPack>>> vExtKeyPacks;
    if (!pwdb->LoadExtKeyPacks(vExtKeyPacks)) {
        LogPrintf("%s -- Error getting ext key packs from wallet db.\n", __func__);
    }
    size_t nExtKeys = 0;
    for (const std::pair<CKeyID, std::vector<CEKAKeyPack>>& keyPack : vExtKeyPacks) {
        CKeyID idAccount = keyPack.first;
        std::vector<CEKAKeyPack> ekPak = keyPack.second;
        ExtKeyAccountMap::iterator mi = mapExtAccounts.find(idAccount);
        if (mi == mapExtAccounts.end()) {
            LogPrintf("%s -- Warning: Unknown ext key account %s.\n", __func__, HDAccIDToString(idAccount));
            continue;
        }
        CExtKeyAccount *sea = mi->second;
    ...
    LogPrintf("%s -- Loaded %d ext pack key%s.\n", __func__, nExtKeys, nExtKeys == 1 ? "" : "s");
    // Load Ext Stealth Key Packs (CEKAStealthKeyPack)
    std::vector<std::pair<CKeyID, std::vector<CEKAStealthKeyPack>>> vExtStealthKeyPacks;
    if (!pwdb->LoadExtStealthKeyPacks(vExtStealthKeyPacks)) {
        LogPrintf("%s -- Error getting ext key packs from wallet db.\n", __func__);
    }
    size_t nStealthKeys = 0;
    for (const std::pair<CKeyID, std::vector<CEKAStealthKeyPack>>& keyPack : vExtStealthKeyPacks) {
        CKeyID idAccount = keyPack.first;
        std::vector<CEKAStealthKeyPack> aksPak = keyPack.second;
        ExtKeyAccountMap::iterator mi = mapExtAccounts.find(idAccount);
        if (mi == mapExtAccounts.end()) {
            LogPrintf("%s -- Warning: Unknown ext key account %s.\n", __func__, HDAccIDToString(idAccount));
            continue;
    ...
    LogPrintf("%s -- Loaded %d stealth pack key%s.\n", __func__, nStealthKeys, nStealthKeys == 1 ? "" : "s");
    // Load Shared Stealth Key Packs (CEKASCKeyPack)
    std::vector<std::pair<CKeyID, std::vector<CEKASCKeyPack>>> vStealthSharedKeyPacks;
    if (!pwdb->LoadSharedStealthKeyPacks(vStealthSharedKeyPacks)) {
        LogPrintf("%s -- Error getting ext key packs from wallet db.\n", __func__);
    }
    size_t nSharedKeys = 0;
    for (const std::pair<CKeyID, std::vector<CEKASCKeyPack>>& keyPack : vStealthSharedKeyPacks) {
        CKeyID idAccount = keyPack.first;
        std::vector<CEKASCKeyPack> asckPak = keyPack.second;
        ExtKeyAccountMap::iterator mi = mapExtAccounts.find(idAccount);
        if (mi == mapExtAccounts.end()) {
            CDynamicAddress addr;
            addr.Set(idAccount, CChainParams::EXT_ACC_HASH);
            LogPrintf("%s -- Warning: Unknown account %s.\n", __func__, addr.ToString());
            continue;
        }
        CExtKeyAccount *sea = mi->second;
    ...
    LogPrintf("%s -- Loaded %d shared key%s.\n", __func__, nSharedKeys, nSharedKeys == 1 ? "" : "s");
...
int CWallet::LoadStealthAddresses()
{
    ...
    for (const std::pair<CKeyID, CStealthAddress>& keyPack : vStealthAddresses) {
        CStealthAddress sx = keyPack.second;
        stealthAddresses.insert(sx);
    }
    LogPrintf("%s -- Loaded %d stealth address key%s.\n", __func__, stealthAddresses.size(), stealthAddresses.size() == 1 ? "" : "s");
...
{
    LogPrintf("%s Preparing Lookahead pools. \n", __func__);
